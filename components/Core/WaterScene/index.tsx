...
const WaterScene: React.FC<WaterSceneProps> = ({ config, initialCameraState, sceneController, isSplitView }) => {
  ...
  useEffect(() => {
    ...
    const animate = () => {
        ...
        const lerpFactor = 0.02; // Smoothness of transitions
        ...
        if (bedMat) {
            tempColor.set(target.colorDeep);
            bedMat.uniforms.uColorDeep.value.lerp(tempColor, lerpFactor);
            tempColor.set(target.colorShallow);
            bedMat.uniforms.uColorShallow.value.lerp(tempColor, lerpFactor);
            bedMat.uniforms.uLightIntensity.value = THREE.MathUtils.lerp(bedMat.uniforms.uLightIntensity.value, target.underwaterLightIntensity, lerpFactor);
            bedMat.uniforms.uCausticsIntensity.value = THREE.MathUtils.lerp(bedMat.uniforms.uCausticsIntensity.value, target.causticsIntensity, lerpFactor);
            bedMat.uniforms.uCausticsScale.value = THREE.MathUtils.lerp(bedMat.uniforms.uCausticsScale.value, target.causticsScale, lerpFactor);
            bedMat.uniforms.uCausticsSpeed.value = THREE.MathUtils.lerp(bedMat.uniforms.uCausticsSpeed.value, target.causticsSpeed, lerpFactor);
            tempColor.set(target.underwaterFogColor);
            bedMat.uniforms.uFogColor.value.lerp(tempColor, lerpFactor);
            bedMat.uniforms.uFogNear.value = THREE.MathUtils.lerp(bedMat.uniforms.uFogNear.value, target.fogCutoffStart, lerpFactor);
            bedMat.uniforms.uFogFar.value = THREE.MathUtils.lerp(bedMat.uniforms.uFogFar.value, target.fogCutoffEnd, lerpFactor);
        }

        // --- UPDATE RAY UNIFORMS ---
        if (rayMat) {
            tempColor.set(target.colorShallow);
            rayMat.uniforms.uColor.value.lerp(tempColor, lerpFactor);
            rayMat.uniforms.uLightIntensity.value = THREE.MathUtils.lerp(rayMat.uniforms.uLightIntensity.value, target.underwaterLightIntensity, lerpFactor);
            if (rayMat.uniforms.uCameraPos) {
                rayMat.uniforms.uCameraPos.value.copy(camera.position);
            }
        }
        ...
        // Always maintain the environment/skybox (User request: don't shift environment when underwater)
        ...
        if (isUnderwater.current) {
            ...
            if (!scene.fog) { // create fog if it doesn't exist
                ...
                if (scene.fog instanceof THREE.Fog) {
                    scene.fog.color.lerp(fogColor, lerpFactor);
                    scene.fog.near = THREE.MathUtils.lerp(scene.fog.near, target.fogCutoffStart, lerpFactor);
                    scene.fog.far = THREE.MathUtils.lerp(scene.fog.far, target.fogCutoffEnd, lerpFactor);
                }
            }
